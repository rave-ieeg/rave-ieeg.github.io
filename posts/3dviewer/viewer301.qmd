...
# Load subject electrode table
t1 <- read.csv("posts/3dviewer/PAV080.csv")
t2 <- read.csv("posts/3dviewer/PAV081.csv")

coord1 <- data.frame(
  Electrode = t1$Electrode,
  x = t1$MNI152_x,
  y = t1$MNI152_y,
  z = t1$MNI152_z,
  Label = t1$Label,
...
coord2 <- data.frame(
  Electrode = max(t1$Electrode) + t2$Electrode,
  x = t2$MNI152_x,
  y = t2$MNI152_y,
  z = t2$MNI152_z,
  Label = t2$Label,
...
template$set_electrodes(rbind(coord1, coord2))

# set electrode values
v1 <- data.frame(
  Electrode = c(coord1$Electrode, coord2$Electrode),
  subject_code = c(coord1$SubjectCode, coord2$SubjectCode),
  surgical_label = c(t1$LabelPrefix, t2$LabelPrefix)
)
template$template_object$set_electrode_values(v1)
...
``` r
b1 <- ravecore::rave_brain("YAEL/PAV080")
b2 <- ravecore::rave_brain("YAEL/PAV081")
...
# Load subject brain
b1 <- ravecore::rave_brain("YAEL/PAV037")
b2 <- ravecore::rave_brain("YAEL/PAV038")
